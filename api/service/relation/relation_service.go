...
import (
	...
	"tiktok_micro/api/model/social/relation"
	user_service "tiktok_micro/api/service/user"
...
func (r *RelationService) FollowAction(req *relation.DouyinRelationActionRequest) (flag bool, err error) {
	_, err = db2.CheckUserExistById(req.ToUserId)
	if err != nil {
		return false, err
	}
	...
	new_follow_relation := &db2.Follows{
		UserId:     current_user_id.(int64),
		FollowerId: req.ToUserId,
	}
	follow_exist, _ := db2.CheckFollowRelationExist(new_follow_relation)
	if req.ActionType == FOLLOW {
		if follow_exist {
			return false, errno.FollowRelationAlreadyExistErr
		}
		flag, err = db2.AddNewFollow(new_follow_relation)
		// 增加redis缓存功能
	} else {
		if !follow_exist {
			return false, errno.FollowRelationNotExistErr
		}
		flag, err = db2.DeleteFollow(new_follow_relation)
		// 增加redis缓存功能
	}
	return flag, err
}
...
func (r *RelationService) GetFollowList(req *relation.DouyinRelationFollowListRequest) (followerlist []*common.User, err error) {
	_, err = db2.CheckUserExistById(req.UserId)
	if err != nil {
		return nil, err
	}
	...
	dbfollows, err := db2.GetFollowIdList(req.UserId)
	if err != nil {
...
func (r *RelationService) GetFollowerList(req *relation.DouyinRelationFollowerListRequest) ([]*common.User, error) {
	...
	dbfollowers, err := db2.GetFollowerIdList(user_id)
	if err != nil {
...
func (r *RelationService) GetFriendList(req *relation.DouyinRelationFriendListRequest) ([]*relation.FriendUser, error) {
	...
	followerIdList, err := db2.GetFollowerIdList(user_id)
	if err != nil {
		return friendList, err
	}

	for _, id := range followerIdList {
		isFriend, err := db2.QueryFollowExist(user_id, id)
		if err != nil {
			return friendList, err
		}
		if isFriend {
			...
			message, err := db2.GetLatestMessageByIdPair(user_id, id)
			if err != nil {
