...
func (s *FeedService) Feed(req *feed.DouyinFeedRequest) (*feed.DouyinFeedResponse, error) {
	...
	dbVideos, err := db2.GetVideosByLastTime(lastTime)
	if err != nil {
		return resp, err
	}
...
func (s *FeedService) CopyVideos(result *[]*feed.Video, data *[]*db2.Video, userId int64) error {
...
func (s *FeedService) createVideo(data *db2.Video, userId int64) *feed.Video {
	video := &feed.Video{
		Id: data.ID,
	...
	go func() {
		err := *new(error)
		video.FavoriteCount, err = db2.GetFavoriteCount(data.ID)
		if err != nil {
	...
	go func() {
		err := *new(error)
		video.CommentCount, err = db2.GetCommentCountByVideoID(data.ID)
		if err != nil {
	...
	go func() {
		err := *new(error)
		video.IsFavorite, err = db2.QueryFavoriteExist(data.ID, userId)
		if err != nil {
